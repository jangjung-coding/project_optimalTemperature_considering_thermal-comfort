...
plt.show() #이전 subect들과 달리 결측치가 모든 열에서 많이 발견됨 
## df으로 결측치 직접 확인
...
plt.show()
## 따라서 KNNImputer를 이용해 'Vote_time_as_number'이 비슷한 주변을 찾아 결측치 처리가 가능함. 이전 subject1,2,3,4들과 달리 특정 날짜 결측치가 많아서 n_neighbors를 3->6으로 설정
...
plt.boxplot(df[environmental_features_include_solar]) # 'Solar'에서 이상치가 많이 발견됨. 이는 subject5 측정 계절이  11월로 Solar그래프(73번 줄)을 보면 습도가 높고 구름이 많이 낀 날이 많았을 것으로 추정할 수 있음
...
df_subject5.shape #총 4개의 열과 146개의 행으로 이루어져 있음
###############################################################################################
#4. model 만들기
##4-1. 데이터 분리(train, test='therm_sens')
from sklearn.model_selection import train_test_split
X = df_subject5.iloc[:, :3]
y = df_subject5.iloc[:, 3]
