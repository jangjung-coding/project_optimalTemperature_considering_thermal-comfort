...
#subject6은 15개의 열과 222개의 행으로 이루어져 있다.
...
plt.show() #'Heart rate'와 'PantT'에서 결측치가 많이 발견됨
## df으로 결측치 직접 확인
...
plt.boxplot(df[environmental_features_include_solar]) # 'Solar'에서 이상치가 많이 발견됨. 이는 subject6 측정 계절이  12월로 Solar그래프를 보면 습도가 높고 구름이 많이 낀 날이 많았을 것으로 추정할 수 있음
...
df_subject6.shape #총 4개의 열과 222개의 행으로 이루어져 있음
###############################################################################################
#4. model 만들기
##4-1. 데이터 분리(train, test='therm_sens')
from sklearn.model_selection import train_test_split
X = df_subject6.iloc[:, :3]
y = df_subject6.iloc[:, 3]
