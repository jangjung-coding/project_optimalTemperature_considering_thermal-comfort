...
#subject10은 15개의 열과 213개의 행으로 이루어져 있다.
...
plt.show() # 'Heart rate' 부분이 유독 결측치가 많음. 이전 subject와 비교했을 때, 결측치를 가진 열이 15개로 적음
## df으로 결측치 직접 확인
df[df.isnull().any(axis=1)][columns_with_missing_values] # 같은 행에서 결측치가 많이 발견됨. 환경변수 부분만 결측치가 있고 이는 캘리포니아의 작은 날씨변동 폭과 잘 측정된 인체 변수 데이터를 버릴 수 없어서 그냥 두기로 함  
'''
결측치 열 9개
    'mean.Temperature_60',
    'grad.Temperature_60', 
...
->'Solar'는 시간에 영향을 많이 받기에 'Vote_time'에 맞춰서 결측치 처리
    'mean.hr_60',
    'grad.hr_60',
-> 생리학적 변수 'hr'는 비교적 순간적으로 급격한 변화를 띄지 않는 변수로 주변 10개 평균값으로 결측치 처리
'''
## -> 'Temperature', 'Humidity'은 2주 사이에 큰 차이가 없어 평균값으로 결측치 처리
...
## -> 생리학적 변수 'hr'는 비교적 순간적으로 급격한 변화를 띄지 않는 변수로 결측치 앞뒤로 20개씩, 총 40개의 평균값으로 결측치 처리(결측치가 다른 subect에 비해 많아서 10->40개로 늘림)
physiological_features = ['mean.hr_60', 'grad.hr_60']
...
plt.boxplot(df[environmental_features_include_solar]) # 앞선 6명의 subject와 같이 'Solar'에서 이상치가 많이 발견됨
...
print(np.where(z > 2)) # 표준정규분포에서 약 97.7%의 데이터가 평균에서 ±2 표준편차 범위에 위치하는 outlier를 이상치로 판단. 'Heart rate' 부분에 이상치의 폭이 비교적 커서 2로 설정
...
df_subject10.shape #총 4개의 열과 213개의 행으로 이루어져 있음
###############################################################################################
#4. model 만들기
##4-1. 데이터 분리(train, test='therm_sens')
from sklearn.model_selection import train_test_split
X = df_subject10.iloc[:, :3]
y = df_subject10.iloc[:, 3]
