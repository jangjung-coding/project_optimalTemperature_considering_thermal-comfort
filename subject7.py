...
#subject7은 15개의 열과 329개의 행으로 이루어져 있다.
...
## 결측치 확인
print(df.columns[df.isnull().any()].tolist()) #'mean.PantT_60', 'grad.PantT_60', 'sd.PantT_60'에 결측치 있어서 다시 돌림
df[physiological_features] = df[physiological_features].fillna(df[physiological_features].rolling(window=41, min_periods=1, center=True).mean()) #다시 한번 결측치 처리
...
df_subject7.shape #총 4개의 열과 222개의 행으로 이루어져 있음
###############################################################################################
#4. model 만들기
##4-1. 데이터 분리(train, test='therm_sens')
from sklearn.model_selection import train_test_split
X = df_subject7.iloc[:, :3]
y = df_subject7.iloc[:, 3]
