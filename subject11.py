...
#subject11은 15개의 열과 310개의 행으로 이루어져 있다.
...
plt.show()
## df으로 결측치 직접 확인
df[df.isnull().any(axis=1)][columns_with_missing_values] # 같은 행에서 결측치가 많이 발견됨. subject10과 마찬가지로 환경변수 부분만 결측치가 있고 이는 캘리포니아의 작은 날씨변동 폭과 잘 측정된 인체 변수 데이터를 버릴 수 없어서 그냥 두기로 함  
'''
결측치 열 9개
    'mean.Temperature_60',
    'grad.Temperature_60', 
...
## -> 생리학적 변수 'hr'는 비교적 순간적으로 급격한 변화를 띄지 않는 변수로 결측치 앞뒤로 5개씩, 총 10개의 평균값으로 결측치 처리
...
print(np.where(z > 3)) # 표준정규분포에서 약 99.7%의 데이터가 평균에서 ±3 표준편차 범위에 위치하는 outlier를 이상치로 판단. 이상치가 적어 z-score를 3으로 설정
...
df_subject11.shape #총 4개의 열과 310개의 행으로 이루어져 있음
###############################################################################################
#4. model 만들기
##4-1. 데이터 분리(train, test='therm_sens')
from sklearn.model_selection import train_test_split
X = df_subject11.iloc[:, :3]
y = df_subject11.iloc[:, 3]
