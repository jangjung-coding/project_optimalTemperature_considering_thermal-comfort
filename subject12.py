...
#subject12은 30개의 열과 156개의 행으로 이루어져 있다.
...
plt.show() # 결측치 열별 사이에 특별한 패턴이 보이지 않음 
## df으로 결측치 직접 확인
...
'''
## -> 'Temperature', 'Humidity', 'Winvel'은 2주 사이에 큰 차이가 없어 평균값으로 결측치 처리
environmental_features = ['grad.Temperature_60', 'sd.Temperature_60','grad.Humidity_60', 'sd.Humidity_60','grad.Winvel_60', 'sd.Winvel_60']
...
df[['mean.hr_60', 'grad.hr_60', 'sd.hr_60']].tail(3) # 뒤에서 2개의 행에서 지속적으로 결측치가 나옴 -> 제거
df = df.drop(df.index[-2:]) # 뒤에서 2개 행 제거
df.shape[0] # 행의 갯수가 156->154로 줄어듦
...
plt.boxplot(df[environmental_features_include_solar]) # 앞선 subject들과 같이 'Solar'에서 이상치가 많이 발견됨
