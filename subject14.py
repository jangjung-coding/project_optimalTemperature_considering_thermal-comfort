...
#subject14은 15개의 열과 262개의 행으로 이루어져 있다.
...
plt.show() # 'Heart rate'와 'WristT'에 결측치가 많이 발견됨 
## df으로 결측치 직접 확인
...
plt.show() # 여름철인 5월에 측정을 해서 태양이 빨리 뜨는 것('Solar'곡선이 비교적 빨리 최고점에 도달함)을 볼 수 있음
...
df_subject14.shape #총 4개의 열과 262개의 행으로 이루어져 있음
###############################################################################################
#4. model 만들기
##4-1. 데이터 분리(train, test='therm_sens')
from sklearn.model_selection import train_test_split
X = df_subject14.iloc[:, :3]
y = df_subject14.iloc[:, 3]
