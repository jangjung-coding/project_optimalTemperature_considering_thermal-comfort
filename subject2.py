...
#subject2은 15개의 열과 229개의 행으로 이루어져 있다.
...
plt.boxplot(df[environmental_features_include_solar]) # 'Solar'에서 이상치가 많이 발견됨. 이는 subject2 측정 계절인 4월에 비가 온 날이 많기 때문으로 판단
...
df_subject2.shape #총 4개의 열과 146개의 행으로 이루어져 있음
###############################################################################################
#4. model 만들기
##4-1. 데이터 분리(train, test='therm_sens')
from sklearn.model_selection import train_test_split
X = df_subject2.iloc[:, :3]
y = df_subject2.iloc[:, 3]
...
plt.xlabel('Value')
plt.show()
## -> LinearRegression, RandomForestRegressor가 MAE, RMSE가 낮음